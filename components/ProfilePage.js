...
const ProfilePage = ({ navigation }) => {
      ...
      if (user) {
        ...
        try {
          ...
          if (userDoc.exists) {
            ...
            setHeight(userData.height !== undefined ? String(userData.height) : '');
            setWeight(userData.weight !== undefined ? String(userData.weight) : '');
          }
        } catch (error) {
  ...
  const handleUpdateProfile = async () => {
    ...
    if (isNaN(parsedHeight) || isNaN(parsedWeight) || parsedHeight <= 0 || parsedWeight <= 0) {
      Alert.alert('Error', 'Height and Weight must be valid positive numbers.');
      setLoading(false);
      return;
    }
