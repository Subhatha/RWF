...
function ProgressScreen() {
  ...
  const [totalWorkoutDuration, setTotalWorkoutDuration] = useState(0); // Store total workout time
    ...
    const fetchProgress = async () => {
      try {
        ...
        if (user) {
          ...
          // Fetch workout data to calculate total workout time
          const workoutsSnapshot = await firebase.firestore().collection('workouts')
          ...
          setTotalWorkoutDuration(totalDuration); // Store total workout duration
        }
      } catch (error) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.summaryContainer}>
        ...
        <Text style={styles.progressText}>
          Total Time Spent Working Out: {totalWorkoutDuration.toFixed(2)} minutes
        </Text>
      </View>
