...
const WorkoutScreen = () => {
  ...
  const findWorkoutPlan = useCallback((user) => {
    ...
    return workoutData.find((plan) => {
      return (
        isInRange(age, plan.Age) &&
        isInRange(height, plan.Height) &&
        isInRange(weight, plan.Weight) &&
        plan.BodyType?.toLowerCase() === bodyType
      );
    });
  }, []);

  // Fetch completed workouts from Firestore on component mount
  useEffect(() => {
  ...
  const markWorkoutDone = async (exercise, repsOrTime) => {
    try {
      ...
      console.log('Marking as done:', exercise);
          ...
          return (
            <View style={styles.exerciseItem}>
              ...
              <TouchableOpacity
                style={[
                  styles.doneButton,
                  isDone && { backgroundColor: '#00cc66' }, // green when done
                ]}
                onPress={() => {
                  if (!isDone) {
